#!/usr/bin/env python3
"""Materialize Run 3 worktrees + prompts for the cross-language (JS/Go) benchmark.
...
is global), and write a ``.rex_prompt.md`` assembled from ``prompts/run-3/``.

Usage:
    python scripts/setup_run3.py                 # create worktrees + deps + prompts
    python scripts/setup_run3.py --prompts-only  # rewrite prompts only (worktrees must exist)
    python scripts/setup_run3.py --repos zod chi # restrict to a subset
"""
...
def main() -> int:
    ...
    print(f"\nRun 3 ready: {len(written)} worktrees configured.")
    for w in written:
        print(f"  {w}")
    return 0
