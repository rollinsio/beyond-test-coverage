#!/usr/bin/env python3
"""Materialize Run 2 worktrees, prompts, and start.sh files.

Run 2 worktrees are named ``wt-r2-<policy>`` so they don't collide with
Run 1's preserved ``wt-<policy>`` worktrees. Branches are
``rex-r2-wt-<policy>``.

Usage:
    python scripts/setup_run2.py                # creates worktrees + prompts + start.sh
    python scripts/setup_run2.py --prompts-only # just writes prompts/start.sh; worktrees must exist

After running, ``scripts/launch_all.sh`` cannot be reused as-is —
update the worktree list in it (or write a ``launch_run2.sh``) to
...
REPOS = {
    ...
    "requests": {
        ...
        "framework_primitives": (
            ...
            "**Used by the Run 1 winning suite (`requests/wt-iter20`)** for "
