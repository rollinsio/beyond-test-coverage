#!/usr/bin/env python3
"""Score the cross-language generated suites vs baselines (multi-language scorer).

Independent recompute (the authoritative cross-language numbers): measures each
``wt-r3-<policy>`` suite and its repo's baseline using the bundled ``score.py``
per-language profiles, and emits ``results-cross-language-scorecard.{json,md}``
in the ``{baselines, arms}`` shape the dashboard consumes — the same shape
``score_quality.py`` produces for Python.

Run AFTER the generation workflow finishes:
    python scripts/score_cross_language.py
"""
...
def main() -> int:
    ...
    # --- markdown ---
    scored = AXES
    md = ["# Cross-language scorecard — JS/TS + Go (gen vs baseline)", "",
    ...
    (ROOT / "results-cross-language-scorecard.md").write_text("\n".join(md) + "\n")

    present = sum(1 for r in arms if r.get("present"))
    better = sum(1 for r in arms if r.get("better"))
    print(f"wrote results-cross-language-scorecard.{{json,md}}  "
