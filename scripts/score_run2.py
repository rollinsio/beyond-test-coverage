#!/usr/bin/env python3
"""Independent Run-2 scorecard recompute.

The Run-2 sessions each wrote their own ``scorecard.json`` / ``final_scorecard.json``
in mutually incompatible shapes (axes as dict-of-{baseline,suite,result}, as
dict-of-{mine,baseline,result}, as flat ``A1: 0`` counts, as grouped A/B/C/D
blocks, ...). Trusting those is both fragile (8 bespoke adapters) and circular
(it trusts each session to score itself).
...
recipes from ``prompts/run-2/quality_scorecard.md``. It then scores each
...
Usage:
    python scripts/score_run2.py                 # all wt-r2-* vs baselines
    python scripts/score_run2.py --run 1         # wt-* (Run 1) vs baselines
"""
...
# Measurement recipes — match prompts/run-2/quality_scorecard.md as closely as a
# single regex pass allows. Counts are total occurrences across all *.py files.
PATTERNS = {
...
def main() -> int:
    ...
    ap.add_argument("--label", default=None,
                    help="Override worktree prefix: wt-<label>-<policy> (e.g. r2b for the "
                         "Opus-4.7 control). Takes precedence over --run.")
    args = ap.parse_args()
    prefix = f"wt-{args.label}-" if args.label else ("wt-" if args.run == 1 else f"wt-r{args.run}-")
