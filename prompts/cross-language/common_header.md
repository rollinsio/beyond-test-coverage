# Benchmark session — {REPO} / {POLICY} (cross-language)
...
worktree created specifically for this run. **The cross-language experiment extends the
...
the way it did in Python (quality: 9/9).
...
## Hard constraints (coverage/quality lessons — do not violate)
...
1. **Do not recover the deleted tests from git.** You will delete and
   ...
   benchmark measures *generation*, not restoration. (The coverage-driven baseline's
