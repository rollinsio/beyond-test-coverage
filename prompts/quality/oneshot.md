## Your task — ONE-SHOT policy (quality)
...
2. Delete the entire `{TESTS_DIR}/` directory and commit the deletion:
   ```bash
   git rm -r {TESTS_DIR}
   git commit -m "Remove tests for the benchmark (oneshot)"
   ```
...
8. Compute your own scorecard. Write it to
   ...
   `git add {TESTS_DIR} .rex_metrics && git commit -m "Generated tests (oneshot)"`.
