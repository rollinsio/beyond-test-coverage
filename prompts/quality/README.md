...
Drafted 2026-05-28 from [coverage-driven baseline findings](../../FINDINGS.md). Worktrees
for this run are named `wt-r2-<policy>` so they don't collide with
the coverage-driven baseline's preserved `wt-<policy>` worktrees.

## The single most important change vs the coverage-driven baseline

**The coverage-driven baseline used coverage as the goal.** Sessions stopped at coverage
parity, then sat on their budget. Generated suites had high coverage
and low quality.

**The quality-driven stage uses a multi-axis quality scorecard as the goal.** Coverage
...
`scripts/setup_quality.py` (in the project root) materializes these
into each worktree as `.rex_prompt.md` and writes `start.sh`.

## Key differences from the coverage-driven prompts (full list)
...
## Falsifiable predictions for the quality-driven stage

If the quality-driven prompts work, we predict:

1. **Mock-real-LOC** (`MagicMock`/`patch(`/`mocker`) trends to zero across all 9 worktrees.
2. **Substring-match assertions** drop by >80 % vs the coverage-driven baseline (count per worktree).
...
6. **iter20 iterations used rises** from the coverage-driven baseline's 2–3 toward the 20 budget — specifically because matching coverage no longer satisfies the stop condition.
...
If any prediction fails, that's a finding for the cross-language experiment.
...
and produce a cross-worktree comparison. That's quality-driven setup work, not
session-level work.
