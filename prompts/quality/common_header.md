# Benchmark session — {REPO} / {POLICY} (quality)
...
This is the **quality-driven** stage. The coverage-driven baseline produced patterns we now know to avoid; the
constraints below encode that. Don't deviate from them.

## The goal

**Produce a test suite that is *better* than the baseline's.** Not a
suite that hits a coverage number. The coverage-driven baseline's framing ("beat baseline
...
the quality-driven stage is quality, measured by the scorecard in `quality_scorecard.md`.
...
coverage to record paths from the wrong worktree. (Discovered in the
coverage-driven baseline: `requests/base/.venv` ended up pointing at `wt-iter20/src/`.)
{DEP_NOTE}
...
The benchmark measures *generation*, not restoration. In the coverage-driven baseline, the
