## Your task — ITER20 policy (quality)
...
Lesson from the coverage-driven baseline: all three iter20 sessions stopped at iteration 2–3
because they hit coverage parity. In the quality-driven stage, the iteration budget is
...
4. **Iterations 2..20** — each iteration must produce a **real
   ...
   **Things that do NOT count as a real improvement:**
   ...
   - Adding tests for `None`-valued mounts, unknown ASGI message
     ...
     coverage-chasing in the coverage-driven baseline).
...
that mattered. This is what the coverage-driven baseline's iter20 didn't produce; if you
