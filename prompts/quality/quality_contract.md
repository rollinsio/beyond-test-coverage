...
The coverage-driven baseline produced suites that hit high coverage but were measurably more
fragile than the human-written baselines. The rules below come
directly from the patterns found in the coverage-driven baseline. Each has a negative form
...
`assert signed == "[42].-9cNi0CxsSB3hZPNCe9a2eEs1ZM"`. Coverage-driven baseline audits
...
In the coverage-driven baseline, multiple iter_2 sessions called `_loads_unsafe_impl`
...
The coverage-driven baseline suites had whole files (`test_exc.py`) that were
...
The coverage-driven baseline suites averaged **2.3–2.5× the LOC of the baseline** because
...
Coverage-driven baseline audits independently predicted three mutations that survive 100
...
The coverage-driven baseline standout (`requests/wt-iter20`, the only result that's
